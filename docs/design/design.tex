...
\textbf{Note on Descriptor Passing Implementation:} While the project requirements suggest a shared memory queue for connection distribution, standard POSIX process isolation prevents the direct sharing of open file descriptors (sockets) purely through shared memory integers. Therefore, this architecture implements:
\begin{itemize}
    \item \textbf{Shared Memory Queue:} Used for synchronization (managing empty/filled slots) and task assignment (tracking which worker is busy).
    \item \textbf{Unix Domain Sockets:} Used as the transport mechanism to securely pass the actual socket file descriptor from Master to Worker using \texttt{SCM\_RIGHTS} ancillary data.
\end{itemize}
...
The \texttt{request\_queue\_t} structure implements a fixed-size FIFO
buffer shared between the master (producer) and workers (consumers).  
Access is synchronized with three semaphores:

\begin{itemize}
    \item \textbf{\texttt{empty\_slots}} — counts free positions; producer blocks when full.
    \item \textbf{\texttt{filled\_slots}} — counts occupied positions; consumers block when empty.
    \item \textbf{\texttt{queue\_mutex}} — ensures exclusive updates of queue indices.
\end{itemize}

This mechanism implements a classic producer–consumer model, ensuring
atomic enqueue and dequeue operations across processes.
...
During normal execution, the Master accepts new client connections and
