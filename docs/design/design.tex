...
\end{figure}
    ...
    \paragraph{JSON Response Format.}\mbox{}\\[-0.2\baselineskip]
    The \texttt{/api/stats} endpoint returns a JSON object with the following structure:
    \begin{lstlisting}[language=C]
    {
      "total_requests": <long>,
      ...
      "status_codes": { "200": <long>, "404": <long>, "500": <long> },
      "cache": { "hits": <size_t>, "misses": <size_t>, "hit_rate": <double> }
    }
    \end{lstlisting}

    \paragraph{Frontend Technologies.}\mbox{}\\[-0.2\baselineskip]
    The dashboard (\texttt{www/dashboard.html}) is a self-contained HTML page that uses:
    \begin{itemize}
      \item \textbf{TailwindCSS} (via CDN) for responsive dark-themed styling.
      \item \textbf{Chart.js} (via CDN) for rendering real-time line charts of requests per second.
      \item Inline JavaScript that polls \texttt{/api/stats} every 2 seconds asynchronously.
    \end{itemize}

    The dashboard displays live metrics including total requests, data transferred, average response time, active connections, HTTP status code distribution, and cache performance (hits, misses, hit rate).
