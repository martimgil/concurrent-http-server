...
\end{figure}
    ...
    Consumer path (in \texttt{worker.c}): wait \texttt{filled\_slots} $\rightarrow$ lock \texttt{queue\_mutex} $\rightarrow$ dequeue $\rightarrow$ unlock $\rightarrow$ post \texttt{empty\_slots}.
        ...
        % --- Columns ---
        \node (master) {\textbf{Master Process} \\ (Producer)};
        \node (shm) [right=4cm of master] {\textbf{Shared Memory} \\ (Critical Section)};
        \node (worker) [right=4cm of shm] {\textbf{Worker Process} \\ (Consumer)};

        % --- Time lines ---
        \draw[thick, gray, dashed] (master) -- ++(0,-9);
        \draw[thick, gray, dashed] (shm) -- ++(0,-9);
        \draw[thick, gray, dashed] (worker) -- ++(0,-9);
        ...
        \node[block, fill=green!10, right=8cm of m_signal, yshift=-1cm] (w_wait) {\texttt{sem\_wait}\\(filled)};
...
\begin{figure}[H]
    ...
    % 4. Cache Decision
    \node (decide) [decision, below of=worker, yshift=-0.5cm] {In Cache?};
    
    % 5. Branches
    \node (read_disk) [process, right=3cm of decide] {Read from Disk};
    \node (update_cache) [process, below of=read_disk] {Update Cache (Write Lock)};
    
    \node (read_mem) [process, left=3cm of decide] {Read from Memory (Read Lock)};
    
    % 6. Response
    \node (build) [process, below=3cm of decide] {Build HTTP Response};
    ...
    \draw [arrow] (accept) -- node[right] {FD via Unix Socket} (worker);
    \draw [arrow] (worker) -- (decide);
    
    % Cache Miss Path
    \draw [arrow] (decide) -- node[above] {No (Miss)} (read_disk);
    \draw [arrow] (read_disk) -- (update_cache);
    \draw [arrow] (update_cache) |- (build);
    
    % Cache Hit Path
    \draw [arrow] (decide) -- node[above] {Yes (Hit)} (read_mem);
