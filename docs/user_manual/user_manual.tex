...
\texttt{LOG\_LEVEL} & INFO &
Determines the verbosity of runtime logging.  
Accepted values: \texttt{DEBUG}, \texttt{INFO}, \texttt{WARN}, and \texttt{ERROR}.  
Higher levels reduce log volume and are suitable for production environments. \\ \hline
...
The project includes an optional static dashboard, provided in the file
\texttt{www/dashboard.html}. This page offers a lightweight browser-based
interface for monitoring server activity and performance statistics.
...
\end{verbatim}
The dashboard is served like any other static file under the directory
defined by \texttt{DOCUMENT\_ROOT} in \texttt{server.conf}.  
No additional configuration or ports are required.

\paragraph{Contents and Functionality.}
The dashboard page uses the shared stylesheet
\texttt{www/style.css} and the JavaScript logic defined in
\texttt{www/script.js} (and its corresponding TypeScript source
\texttt{script.ts}).  
It may display or periodically refresh basic metrics such as:
\begin{itemize}
  \item Total requests handled by the server.
  \item Active workers and threads in operation.
  \item Cache hit and miss ratios.
  \item Current uptime or activity counters.
\end{itemize}

\paragraph{Troubleshooting.}
\begin{itemize}
  \item If the browser reports a \texttt{404 Not Found}, ensure that
        \texttt{dashboard.html} exists inside the configured document root:
        \begin{lstlisting}[language=bash]
        ls www/ | grep dashboard.html
        \end{lstlisting}
  \item If the page shows no data, verify that any referenced statistics endpoint
        (for example, \texttt{/api/stats}) is reachable:
        \begin{lstlisting}[language=bash]
        curl http://localhost:8080/api/stats
        \end{lstlisting}
  \item If changes to the page or scripts are not visible, clear the browser cache
        or force a reload with \texttt{Ctrl + F5}.
\end{itemize}

\paragraph{Summary.}
The Web Dashboard is an optional, static component intended to give users a
simple graphical overview of the server’s current state.
It is always accessible at:
